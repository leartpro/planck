...
unsigned int Lexer::transition_delta(int s, char sigma) {
    ...
    switch(sigma){
        ...
        default:
            auto ascii = (int) (unsigned char)sigma;
...
Token Lexer::next_token(std::string &program, unsigned int &current_index) {
    ...
    if(current_index == program.length()){
        lexeme = (char) EOF;
        current_index++;
        return {22, lexeme, get_line_number(program, current_index)};
    }
    ...
    while(current_state != e){
        ...
        current_state = (signed int) transition_delta(current_state, current_symbol);
    ...
    if(is_final[current_state])
        return {current_state, std::move(lexeme), get_line_number(program, current_index)};
